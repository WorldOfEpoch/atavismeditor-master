...
import {AbilitiesTriggers, AbilitiesTriggersActions, eventType, subActionType} from './abilities-triggers.data';
...
export class AbilitiesTriggersService {
  public tableKey = TabTypes.ABILITIES_TRIGGERS;
  private readonly listStream = new BehaviorSubject<AbilitiesTriggers[]>([]);
  public list = this.listStream.asObservable();
  public dbProfile!: DataBaseProfile;
  public dbTable = abilitiesTriggersProfileTable;
  public dbActionsTable = abilitiesTriggersActionsTable;
  ...
  public async getList(queryParams: QueryParams): Promise<void> {
    const response = await this.databaseService.queryList<AbilitiesTriggers>(
      this.dbProfile,
      this.dbTable,
  ...
  public async addItem(): Promise<null | DropdownValue> {
    ...
    let {item} = await this.tablesService.openDialog<AbilitiesTriggers>(formConfig, form, {actions: this.actionsForm});
    if (!item) {
      this.resetForm(form);
    ...
    const actions = item.actions as AbilitiesTriggersActions[];
    delete item.actions;
    item = this.setDefaults(item);
    const newId = await this.databaseService.insert<AbilitiesTriggers>(this.dbProfile, this.dbTable, item);
    this.resetForm(form);
  ...
  public async updateItem(id: number): Promise<null | DropdownValue> {
    ...
    const record = await this.databaseService.queryItem<AbilitiesTriggers>(this.dbProfile, this.dbTable, 'id', id);
    if (!record) {
      return null;
    }
    const list: AbilitiesTriggersActions[] = await this.databaseService.customQuery(
      this.dbProfile,
      `SELECT * FROM ${this.dbActionsTable} WHERE abilities_triggers_id = ?`,
      [record.id],
    );
    ...
    if (action === DialogCloseType.save_as_new) {
      delete item.id;
      item.creationtimestamp = this.databaseService.getTimestampNow();
      newId = await this.databaseService.insert<AbilitiesTriggers>(this.dbProfile, this.dbTable, item);
      await this.saveSubs(newId, actions.map((act) => ({...act,id: undefined})), []);
    } else {
      await this.databaseService.update<AbilitiesTriggers>(this.dbProfile, this.dbTable, item, 'id', record.id as number);
  ...
  private setDefaults(item: AbilitiesTriggers): AbilitiesTriggers {
    item.isactive = true;
  ...
  private async saveSubs(recordId: number, items: AbilitiesTriggersActions[], itemsAll: number[]): Promise<void> {
    for (const item of items) {
      ...
      item.abilities_triggers_id = recordId;
      if (item.id) {
        itemsAll.splice(itemsAll.indexOf(item.id), 1);
        await this.databaseService.update<AbilitiesTriggersActions>(
          this.dbProfile,
          this.dbActionsTable,
          item,
          'id',
        ...
        // @ts-ignore
        delete item.id;
        await this.databaseService.insert<AbilitiesTriggersActions>(this.dbProfile, this.dbActionsTable, item, false);
      }
    }
  ...
  public async duplicateItem(id: number): Promise<number> {
    const baseRecord = await this.databaseService.queryItem<AbilitiesTriggers>(this.dbProfile, this.dbTable, 'id', id);
    const record = {...baseRecord};
    delete record.id;
    record.name = record.name + ' (1)';
    let list: AbilitiesTriggersActions[] = await this.databaseService.customQuery(
      this.dbProfile,
      `SELECT * FROM ${this.dbActionsTable} WHERE abilities_triggers_id = ?`,
      [baseRecord.id],
    );
    ...
    const actions = item.actions as AbilitiesTriggersActions[];
    delete item.actions;
    const newId = await this.databaseService.insert<AbilitiesTriggers>(this.dbProfile, this.dbTable, item, false);
    await this.saveSubs(newId, actions, []);
  ...
  private async prepareForm(
    record: AbilitiesTriggers,
    list: AbilitiesTriggersActions[],
    updateMode = false,
  ): Promise<{item: AbilitiesTriggers | undefined, action: DialogCloseType}> {
    this.formConfig.title = this.translate.instant(this.tableKey + '.EDIT_TITLE');
    this.formConfig.submit = this.translate.instant('ACTIONS.UPDATE');
    // if ([eventType.dodge, eventType.miss, eventType.parry].includes(record.event_type)) {
    ...
    for (const action of list) {
      ...
      (form.get('actions') as FormArray).push(
        this.subFormService.buildSubForm<AbilitiesTriggersActions, any>(actionForm, action),
      );
    }
    form.patchValue(record);
    formConfig.saveAsNew = updateMode;
    let {item, action} = await this.tablesService.openDialog<AbilitiesTriggers>(formConfig, form, {
