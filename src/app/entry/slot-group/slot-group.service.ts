...
import {slotsGroupTable, slotsInGroupTable} from '../tables.data';
...
import {SlotGroup, SlotInGroup} from './slot-group.data';
...
export class SlotGroupService {
  ...
  public tableKey = TabTypes.SLOT_GROUP;
  private readonly listStream = new BehaviorSubject<SlotGroup[]>([]);
  public list = this.listStream.asObservable();
  public dbProfile!: DataBaseProfile;
  public dbTable = slotsGroupTable;
  public dbTableSlot = slotsInGroupTable;
  ...
  public async getList(queryParams: QueryParams): Promise<void> {
    this.dropdownItemsService.getSlotNames();
    const response = await this.databaseService.queryList<SlotGroup>(
      this.dbProfile,
      this.dbTable,
  ...
  public async addItem(): Promise<null | DropdownValue> {
    ...
    let {item} = await this.tablesService.openDialog<SlotGroup>(formConfig, form);
    if (!item) {
      this.resetForm(form);
    ...
    const newId = await this.databaseService.insert<SlotGroup>(this.dbProfile, this.dbTable, item);
  ...
  public async updateItem(id: number): Promise<null | DropdownValue> {
    const record = await this.databaseService.queryItem<SlotGroup>(this.dbProfile, this.dbTable, 'id', id);
    if (!record) {
      return null;
    }
    const list: SlotInGroup[] = await this.databaseService.customQuery(
      this.dbProfile,
      `SELECT * FROM ${this.dbTableSlot} WHERE slot_group_id = ?`,
      [record.id],
      false,
    );
    ...
    if (action === DialogCloseType.save_as_new) {
      ...
      item.isactive = true;
      item.creationtimestamp = this.databaseService.getTimestampNow();
      newId = await this.databaseService.insert<SlotGroup>(this.dbProfile, this.dbTable, item);
      await this.addSubs(newId, slots);
    } else {
      await this.databaseService.update<SlotGroup>(this.dbProfile, this.dbTable, item, 'id', record.id as number);
  ...
  public defaults(item: SlotGroup): SlotGroup {
  ...
  private async addSubs(id: number, items: string[]): Promise<void> {
    for (const item of items) {
      const newItem: SlotInGroup = {
        slot_group_id: id,
        slot_id: +item,
      };
      await this.databaseService.insert<SlotInGroup>(this.dbProfile, this.dbTableSlot, newItem, false);
    }
  }
  ...
  private async saveSubs(id: number, items: string[], itemsAll: number[] = []): Promise<void> {
    for (const item of items) {
      const itemResult = await this.databaseService.customQuery(
        this.dbProfile,
        `SELECT * FROM ${this.dbTableSlot} WHERE slot_group_id = ? and slot_id = ?`,
        [id, +item],
        false,
      );
  ...
  private async prepareForm(record: SlotGroup, updateMode = false): Promise<{item: SlotGroup | undefined, action: DialogCloseType}> {
    ...
    const {item, action} = await this.tablesService.openDialog<SlotGroup>(formConfig, form);
    if (!item) {
      this.resetForm(form);
      this.tablesService.dialogRef = null;
  ...
  public async duplicateItem(id: number): Promise<number> {
    const duplicatedRecord = await this.databaseService.queryItem<SlotGroup>(this.dbProfile, this.dbTable, 'id', id);
    ...
    delete item.slot_id;
    item = this.defaults(item);
    const newId = await this.databaseService.insert<SlotGroup>(this.dbProfile, this.dbTable, item, false);
    await this.addSubs(newId, slots);
